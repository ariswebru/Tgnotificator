...
#!/usr/bin/env python3
"""
Telegram Bot для уведомлений о задачах
        ...
        try:
            ...
            if not telegram_username:
                await update.message.reply_text(
                    ...
                    "Убедитесь, что он установлен в настройках Telegram."
                )
                return
            ...
            if not user_response.data:
                await update.message.reply_text(
                    ...
                    "Убедитесь, что вы указали корректный Telegram username в профиле."
                )
                return
            ...
            # Создаем inline keyboard для действий
            keyboard = []
            for task in tasks_response.data[:5]:  # Показываем кнопки только для первых 5 задач
                keyboard.append([
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if callback_data.startswith('show_task_'):
                # Формат: show_task_{task_id}_{user_id}
                parts = callback_data.split('_')
                if len(parts) >= 4:
                    task_id = parts[2]
                    user_id = parts[3]
                    
                    # Проверяем, что пользователь имеет право видеть эту задачу
                ...
                # Показать детали задачи из команды /my_tasks
...
🔥 Приоритет: {priority_text}{due_date_text}{estimated_hours}{actual_hours}
        ...
        # Создаем кнопки для изменения статуса (только если задача не завершена)
        keyboard = []
    ...
    async def send_notification_to_group(self, notification_data: Dict):
        """Отправить уведомление в группу"""
        try:
            telegram_username = notification_data.get('telegram_username')
            ...
            message_text = f"{type_emoji.get(notification_type, '📋')} Уведомление для @{telegram_username}"
            
            keyboard = [[
                InlineKeyboardButton(
                    "👀 Показать детали", 
                    callback_data=f"show_task_{task_id}_{user_id}"
                )
            ]]
